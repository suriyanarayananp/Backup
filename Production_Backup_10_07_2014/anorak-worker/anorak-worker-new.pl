...
package AnorakDB;
require "/opt/anorak-worker/AnorakDB.pm";
...
package Landsend_US;
require "/opt/home/merit/Merit_Robots/Landsend_US-new.pm";
		...
		if (defined $process) {
			eval {
				...
				my $rc = timeout $ini->{worker}->{timeout} => sub {
					&$process($decoded->{'ObjectKey'},$decoded->{'url'}, $database, $decoded->{'RobotName'} . '--Worker', $decoded->{'RetailerId'}, $logger);
				} ;
				if ($@) {
					# Report a timeout
					...
					# Transaction Rollback for Timeout
					$database->rollback();
