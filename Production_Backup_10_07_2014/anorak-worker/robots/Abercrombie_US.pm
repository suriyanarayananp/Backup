...
sub Abercrombie_US_DetailProcess()
{
	...
	if($url3 =~ m/kids\.com/is)
	{
		$url3='http://www.abercrombiekids.com'.$url3 unless($url3=~m/^\s*http\:/is);
	}
	else
	{
	...
	if($content2 =~ m/\"swatch\-link\"\s*href\=\"([^>]*?)\"\s*data\-seq\=\"([^>]*?)\"\s*data\-productId\=\"([^>]*?)\"\s*data\-categoryid\=\"([^>]*?)\"[^>]*?title\=\"([^>]*?)\"/is) 
	{
		...
		{ 
			...
			if($url3 =~ m/kids\.com/is)
			{
				$colorURL = "http://www.abercrombiekids.com/webapp/wcs/stores/servlet/GetColorJSON?storeId=$storeid&catalogId=$catlogid&categoryId=$catid&productId=$poid&seq=$dataseq"; 
			}
			else
			{
	...
	{
		my $seqid = $_;
		my $imageCollectionURL;
		if($url3 =~ m/kids\.com/is)
		{
			$imageCollectionURL = "http://anf.scene7.com/is/image/anf?imageset={anf/kids_".$collection_id."_".$seqid.",anf/kids_".$collection_id."_".$seqid."_video1,anf/kids_".$collection_id."_".$seqid."_video2,anf/kids_".$collection_id."_".$seqid."_video3,anf/kids__video1,anf/kids__video2,anf/kids__video3}&req=set,json&defaultimage=anf/kids_".$collection_id."_".$seqid."_prod1&handler=scene7JSONResponse&id=colorSet&callback=jQuery1707709266603571354_1387185889160&_=1387185889637";
		}
		else
		{
