...
sub Bhs_UK_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		my (%sku_objectkey,%image_objectkey,%hash_default_image,@image_object_key,@sku_object_key,@string,$size);	
		...
		elsif($content2 =~ m/item_out_of_stock\s*(?:\"|\')\s*>[^>]*?this\s*item\s*is\s*out\s*of\s*stock/is)
		{
			$out_of_stock='y';
			$price_text=&price_text($price,$was_price1,$off_price1);
			$price='null' if($price eq '' or $price eq ' ');
		...
		if ( $content2 =~ m/<meta\s*property\=\"og\:image\"\s*content\=\"([^>]*?)\"\/\s*>/is )
		{
			...
			if($content2 =~ m/thumbnails:\s*\[([^>]*?)\s*\]/is)
			{
				@string = split(/,/,$1);
			}
		...
		if(($product_name eq '')&&($product_id ne '')&&(!$skuflag))
		{
			$product_name='-';
		}
		if((($description eq '') or ($description eq ' '))&&(($prod_detail eq '') or ($prod_detail eq ' '))&&($product_id ne '')&&(!$skuflag))
		{
			$description='-';
		}
