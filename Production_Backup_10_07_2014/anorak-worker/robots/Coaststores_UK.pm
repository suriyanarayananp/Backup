...
sub Coaststores_UK_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		#Product price text
		if($content2 =~ m/class\=\"product_price\">([\w\W]*?)<\/p>/is)
		{
			$price_text = $1;
			$price_text =~ s/<[^>]*?>/ /igs;
		...
		{
			my $price_text2 = $1;
			$price_text2 =~ s/\&pound\;/£/ig;
			if($price_text2=~m/(\d[^>]*?)\s*<[^>]*?(?:\"|\')\s*was_price\s*(?:\"|\')\s*>/is)
			{
				$price=$1;
			}
			elsif($price_text2=~m/Original\s*\:\s*\£\s*(\d[^>]*?)\s*From/is)
			{
				$price=$1;
			}
			elsif($price_text2=~m/From\s*\:\s*\£\s*(\d[^>]*?)\s*to/is)
			{
				$price=$1;
			}
			elsif($price_text2=~m/\£\s*(\d[^>]*?)$/)
			{
				$price=$1;
			}
			undef($price_text2);
