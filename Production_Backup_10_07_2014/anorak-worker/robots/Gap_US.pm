...
sub Gap_US_DetailProcess()
{
	...
	{		
		...
		my $super_url0="http://www.gap.com/browse/productData.do?pid=$product_id1&vid=1&scid=&actFltr=false&locale=en_US&internationalShippingCurrencyCode=&internationalShippingCountryCode=us&globalShippingCountryCode=us";
		...
		undef ($ckproduct_id);
		###### Size Category (Regular, Tall, Petite)
		...
		#	my $type_id=2;
			my @query_string;
			my $super_url="http://www.gap.com/browse/productData.do?pid=$product_id1&vid=".$type_id."&scid=&actFltr=false&locale=en_US&internationalShippingCurrencyCode=&internationalShippingCountryCode=us&globalShippingCountryCode=us";
			...
			if($content2=~m/SizeInfoSummary\s*\(\s*([\w\W]*?)\s*\)\;/is)
				{
					...
					while($cong23=~m/[:?Size]?[^>]*(?:\,\"([^>]*?)\"\,\"([^>]*?)\",\")[\d]\"\,\"[^>]*?\"\,\"([^>]*?)\"\,\"/igs)
					{
						...
						while($cong231=~m/([\w\(\)(\/)(\-)\s]+)\^\,\^([\d]+)/igs)
						{
							...
							my $var2336=$2;
							$var234=~s/^\://is;
							push(@arey,$var234);	##### Size Type Name #####
							push(@areye,$var2336);	###### Size Type Value ####
						}
					}	
					if($cong23=~m/\|\|[^>]*?\"\,\"([^>]*?)\"\,false/is)
					{
						...
						while ($cong232=~m/([\w\(\)(\/)(\-)\s]+)\^\,\^([\d]+)/igs)
						{
							...
							my $code1=$2;
							$type1=~s/^\://is;
							push(@type,$type1);			##### Size Type Name #####
							push(@code,$code1);			###### Size Type Value ####
						}
					}
				}	
				...
				while($content2=~m/(objP\.StyleColor\s*[\w\W]*?\s*\)\;)/igs)
				{
					my $cont3554=$1;
					$cont3554=~s/<[^>]*?>/ /igs;
					$cont3554=~s/See\s*all//igs;
					if($cont3554=~m/Color\("\s*[^>]*?\s*","\s*([^>]*?)\s*"[\w\W]*?(\$\s*[^>]*?)\"\"\,/is)
					#if($cont3554=~m/Color\("\s*[^>]*?\s*","\s*([^>]*?)\s*"[\w\W]*?([^>]*?undefined)/is)
					{
						$color=$1;
						$price_textblock=$2;
				...
				while ($content2=~m/Swatch\^\,\^([^>]*?)\|/igs)
				{
					...
					my ($imgid,$img_file) = &DBIL::ImageDownload($swatch,'swatch','gap-us');
				...
				while($content2=~m/\'VLI\s*\'\s*:\s*\'([^>]*?)\'/igs)
				{
					...
					my ($imgid,$img_file) = &DBIL::ImageDownload($product_image1,'product','gap-us');
				...
				while($content2=~m/\'AV[\d]_VLI\s*\'\s*:\s*\'([^>]*?)\'/igs)
				{
					...
					my ($imgid,$img_file) = &DBIL::ImageDownload($product_image2,'product','gap-us');
				...
				$dbh->commit();
				$count++;
				# &SaveProduct($hash_url{$url},'','bb65a0776e4135eef2969d3882e8d01a3852f',$scrape_date,$url3,$product_id,$product_name,$brand,$prod_detail,$description,$prod_objkey{$url3});
