...
sub Hollister_US_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		$url3='http://www.hollisterco.com'.$url3 unless($url3=~m/^\s*http\:/is);
		my $content2 = get_content($url3);
		my @query_string;
		goto PNF if($content2 =~ m/PAGE\s*MAY\s*NO\s*LONGER\s*EXIST/is);
		goto PNF if($content2 =~ m/This\s*item\s*is\s*no\s*longer\s*available\./is);
		...
		if($content2 =~ m/\"swatch\-link\"\s*href\=\"([^>]*?)\"\s*data\-seq\=\"([^>]*?)\"\s*data\-productId\=\"([^>]*?)\"\s*data\-categoryid\=\"([^>]*?)\"[^>]*?title\=\"([^>]*?)\"/is)
		{
			while($content2 =~ m/\"swatch\-link\"\s*href\=\"([^>]*?)\"\s*data\-seq\=\"([^>]*?)\"\s*data\-productId\=\"([^>]*?)\"\s*data\-categoryid\=\"([^>]*?)\"[^>]*?title\=\"([^>]*?)\"/igs) 
			{
				...
				push @seqno, $dataseq;
				my $colorURL = "http://www.hollisterco.com/webapp/wcs/stores/servlet/GetColorJSON?storeId=$storeid&catalogId=$catlogid&categoryId=$catid&productId=$poid&seq=$dataseq"; 
		...
		else
		{
			...
			$colorURL = "http://www.hollisterco.com/webapp/wcs/stores/servlet/GetColorJSON?storeId=$storeid&catalogId=$catlogid&categoryId=$catid&productId=$poid&seq=$dataseq"; 
			my $colorCont = get_content($colorURL); 
			push @seqno, $dataseq;
			if($colorCont =~ m/\"items\"\s*\:\s*\[([\w\W]*?)\]/is) 
			{ 
		...
		{
			my $seqid = $_;
			my $imageCollectionURL = "http://anf.scene7.com/is/image/anf?imageset={anf/hol_".$collection_id."_".$seqid.",anf/hol_".$collection_id."_".$seqid."_video1,anf/hol_".$collection_id."_".$seqid."_video2,anf/hol_".$collection_id."_".$seqid."_video3,anf/hol__video1,anf/hol__video2,anf/hol__video3}&req=set,json&defaultimage=anf/hol_".$collection_id."_".$seqid."_prod1&handler=scene7JSONResponse&id=colorSet&callback=jQuery11020884198005301791_1391164043589&_=1391164043590";
