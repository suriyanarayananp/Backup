...
sub Landsend_US_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		my (%sku_objectkey,%image_objectkey,%hash_default_image,%sku_imge_hash,%sku_style_hash,%has_image,%imagetype_hash,%imagecol_hash,%imagedef_hash);
		
		if($content2=~m/<div[^>]*?product\-message[^>]*?>\s*<b>Sorry\,\s*this\s*item\s*is\s*currently\s*unavailable\s*</is)
		{
			goto MP;
		}
		...
		my @def_image;
		while($content2 =~ m/\.productImage\s*\=\s*(ProductImage\d+)/igs)
		{
			my $main_image=$1;
			push(@def_image, $main_image);
		}
		...
		foreach(@imagearry1)
		{
			...
			{
				# if($tempno=~ m/(?:_FF|_LF|_M1)/is)
				if(grep( /\s*$def\s*/,@def_image1))
				{
