...
sub Ms_UK_DetailProcess()
{
	...
	{		
		...
		$description=~s/\&reg\;/\®/igs;			
		$description=~s/\&eacute\;/\é/is;
		$description=~s/\&egrave\;/\é/is;
		$description=~s/\&trade\;/\™/igs;
		$prod_detail=~s/\&reg\;/\®/igs;			
		$prod_detail=~s/\&eacute\;/\é/is;
		$prod_detail=~s/\&egrave\;/\é/is;
		$prod_detail=~s/\&trade\;/\™/igs;
		...
		else
		{
			...
			{
				my $swatchCont = $1;
				while($swatchCont =~ m/data\-swatch\-name\=\"([^>]*?)\"\s*style\=\"background\-image\:url\(([^>]*?)\)|data\-swatch\-name\=\"([^>]*?)\"\s*data\-swatch\-src\=\"([^>]*?)\"/igs)
				{
					my $color = &DBIL::Trim($1.$3);
					my $swatchURL = &DBIL::Trim($2.$4);
							...
							foreach (keys %sizeID)
							{
								...
								if($StockCont =~ m/\"$size\"\:\{\"count\"\:(\d+)\.\d+/is)
								{
									...
									if($content2 =~ m/\"$mainID\_$tcolor\_$size\"\:\{\"price\"\:\"([^>]*?)\"\,\"prevPrice\"\:\"([^>]*?)\"\,\"offerText\"\:\"([^>]*?)\"\,\"unitPrice\"\:\"([^>]*?)\"}/is)
									{
										...
										$pricetext =~ s/<[^>]*?>/ /igs;
										$pricetext =~ s/\&nbsp\;|amp\;/ /igs;
										$pricetext =~ s/\s+/ /igs;
										$pricetext =~ s/^\s+|\s+$//igs;
										$pricetext =~ s/\&pound\;/\Â\£/igs;
										$pricetext =~ s/\&\#163\;/\Â\£/igs;
										$pricetext =~ s/\Â//igs;
