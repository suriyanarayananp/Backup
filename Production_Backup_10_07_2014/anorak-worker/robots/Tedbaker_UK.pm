...
sub Tedbaker_UK_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		if ( $content2 =~ m/<\/hgroup>\s*<ul\s*class\=\"pricing\">([\w\W]*?)<\/ul>/is )
		{
			$price_text =$1;
			$price_text=~s/<[^>]*?>/ /igs;
		...
		my (%sku_objectkey,%image_objectkey,%hash_default_image,@colorsid,@image_object_key,@sku_object_key,$nex_swa,$swcode);

		if ( $content2 =~ m/<div[^>]*?class=\"colours_switch\"[^>]*?>([\W\w]*?)<\/div>/is ) ##To take details of Next Swatch. 
		{
			$nex_swa=$1;
		}
...
Swatch:
		...
		my ($price,$out_of_stock,$color,@color,$size,$count);
		...
		{	
			...
			if($color_block_size =~ m/\"pricetype\"\s*\:\s*\"buy\"\s*\,\s*\"value\"\s*\:\s*([^>]*?)\,/is )
			{
				$price = &DBIL::Trim($1);
			}
