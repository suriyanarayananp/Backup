...
sub Topshop_UK_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		my ($price,$price_text,$brand,$sub_category,$product_id,$product_name,$description,$main_image,$prod_detail,$alt_image,$out_of_stock,$color,$clr,$status,%sku_objectkey,%image_objectkey,%hash_default_image,@colorsid,@image_object_key,@sku_object_key);	
		##Getting Details for Multiple Product Item.
		if($mflag)
		{
		...
		elsif($content2 =~ m/\->\s*<li\s*class\=\s*\"[^<]*product_price\"\s*>\s*([\w\W]*?)\s*<li\s*class=\"product_colour\"/is)
		{
			$price_text=&DBIL::Trim($1);
		...
		if ( $content2 =~ m/<meta\s*property\=\"og\:image\"\s*content\=\s*(?:\"|\')\s*([^\"\']*?)\s*(?:\"|\')[^>]*?>/is )
		{
			...
			$status=get_content_status($imageurl);
			...
			foreach my $count ( 2 .. 5 )
			{
				...
				$status=get_content_status($imageurl1);  
