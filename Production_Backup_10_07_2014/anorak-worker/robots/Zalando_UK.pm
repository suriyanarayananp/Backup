	...
	{		
		...
		if(@colorList)
		{
			foreach(@colorList)
			{
				...
				if($defaulImage eq '')
				{
					$defaulImage = &DBIL::Trim($1) if($content3 =~ m/property\=\"og\:image\"\s*content\=\"([^>]*?)\"/is);
					$defaulImage =~ s/\/detail\//\/large\//igs;
				}
		...
		else
		{
			...
			if($defaulImage eq '')
			{
				$defaulImage = &DBIL::Trim($1) if($content2 =~ m/property\=\"og\:image\"\s*content\=\"([^>]*?)\"/is);
				$defaulImage =~ s/\/detail\//\/large\//igs;
			}
