...
sub Zara_US_DetailProcess()
{
	...
	if($product_object_key)
	{
		...
		my ($image,$price,$price_text,$brand,$sub_category,$item_no,$product_name,$description,$main_image,$prod_detail,$alt_image,$out_of_stock,$colour,$colour11,@colour,$Url1_inside,$size,$blk,@colour1,$clr1,$ckproduct_id);
		...
		if ( $content1 =~ m/Choose\s*a\s*colour\s*<([\w\W]*?)(?:<\/label>\s*<\/div>|<h2)/is )  #Swatch Images Block
		{
			...
			while($blk1=~m/title\s*\=\s*(?:\"|\')([^>]*?)\s*(?:\"|\')[^>]*?>\s*<img[^>]*?src\s*\=\s*\"([^>]*?)\s*\"[^>]*?>/igs)   #Getting Each swatch image urls
			{
				my $swa_clr=$1;
		...
		elsif($content1=~m/<div[^>]*?class\s*\=\s*\"big\s*Image\s*Container\s*\"[^>]*?>(?:(?:\s*<[^>]*?>\s*)+\s*)?\s*<a[^>]*?href\s*\=\s*(?:\"|\')([^>]*?)(?:\"|\')[^>]*?>/is)
		{
			$main_image = $1;
			$main_image =~ s/\$//g;			
